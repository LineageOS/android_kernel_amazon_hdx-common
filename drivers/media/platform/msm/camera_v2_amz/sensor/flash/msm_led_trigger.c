	...
	void *data)
{
	...
	// If someone writes to the LED sysfs entries, it can remove the triggers.
	// Re add if necessary.

	read_lock(&fctrl->torch_trigger->leddev_list_lock);
	empty = list_empty(&fctrl->torch_trigger->led_cdevs);
	read_unlock(&fctrl->torch_trigger->leddev_list_lock);
	if (empty) {
		led_trigger_unregister_simple(fctrl->torch_trigger);
		led_trigger_register_simple(fctrl->torch_trigger_name,
			&fctrl->torch_trigger);
	}
