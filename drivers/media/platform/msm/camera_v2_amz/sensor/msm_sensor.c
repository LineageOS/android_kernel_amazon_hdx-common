...
int32_t msm_sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	pr_debug("%s %d", __func__, on);
	if (!on && s_ctrl->sensor_state == MSM_SENSOR_POWER_UP) {
		/* either mediaserver or mm-qcamera-daemon has died, send
		 * shutdown command to put hardware in sane state for recovery */
		pr_err("%s Emergency Power off CMD", __func__);
		cdata.cfgtype = CFG_POWER_DOWN;
		s_ctrl->func_tbl->sensor_config(s_ctrl, (void*) &cdata);
	}
